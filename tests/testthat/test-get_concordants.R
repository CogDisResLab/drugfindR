...
input_filenames <- expand_grid(signature = c("kd_signature", "oe_signature", "cp_signature"), direction = c("up", "down"), threshold = c(0.85)) %>% unite("filename") %>% pull(filename) %>% str_c(., "csv", sep = ".") %>% file.path("reference", .)

input_data <- map(input_filenames, ~ read_csv(.x, col_types = signature_col_spec))

reference_filenames <- expand_grid(concordant = "concordant", signature = c("kd_signature", "oe_signature", "cp_signature"), direction = c("up", "down"), threshold = c(0.85), output_lib = c("OE", "KD", "CP")) %>% unite("filename") %>% pull(filename) %>% str_c(., "csv", sep = ".") %>% file.path("reference", .)
