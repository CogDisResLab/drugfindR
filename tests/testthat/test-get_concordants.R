...
input_filenames <-
  expand_grid(
    signature = c("kd_signature", "oe_signature", "cp_signature"),
    direction = c("any", "up", "down"),
    threshold = c(0.85)
  ) %>% unite("filename") %>% pull(filename) %>% str_c(., "csv", sep = ".") %>% file.path("reference", .)

input_data <-
  map(input_filenames, ~ read_csv(.x, col_types = signature_col_spec))
...
reference_filenames <-
  expand_grid(
    ...
    direction = c("any", "up", "down"),
    threshold = c(0.85),
    output_lib = c("OE", "KD", "CP")
  ) %>% unite("filename") %>% pull(filename) %>% str_c(., "csv", sep = ".") %>% file.path("reference", .)

reference_data <-
  map(reference_filenames,
      ~ read_csv(.x, col_types = concordant_col_spec, na = c("")))

test_that("Error thrown at invalid signature", {
  expect_error(get_concordants("a"),
               "signature must be a data frame or data frame like object")
})

test_that("Error thrown at invalid library", {
  expect_error(get_concordants(tibble(), "AB"),
               "library must be one of 'OE', 'KD' or 'CP'")
  expect_error(get_concordants(tibble(), "CD"),
               "library must be one of 'OE', 'KD' or 'CP'")
  expect_error(get_concordants(tibble(), "OJ"),
               "library must be one of 'OE', 'KD' or 'CP'")
})
